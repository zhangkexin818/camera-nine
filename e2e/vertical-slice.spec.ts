...
async function finishPrologue(page: Page, memory: 'card' | 'signal' = 'card', controlledClock = false) {
  ...
  await expect(page.locator('.character-dialogue')).toBeVisible()
  for (let index = 0; index < 3; index += 1) await page.locator('.speech-ui > button').click()
  await page.getByRole('button', { name: memory === 'card' ? /保持拍摄/ : /放下机器/ }).click()
...
test('keeps prologue choices and exploration controls usable on a phone', async ({ page }) => {
  ...
  await expect(page.locator('.identity-visual')).toBeVisible()
  for (const record of ['工作证', '楼层扫描', '受损录像']) await page.getByRole('button', { name: record }).click()
  await expect(page.getByRole('button', { name: /相信档案/ })).toBeInViewport()
  await page.getByRole('button', { name: /相信档案/ }).click()
  for (let index = 0; index < 3; index += 1) await page.locator('.speech-ui > button').click()
  await page.getByRole('button', { name: /放下机器/ }).click()
  await page.getByRole('button', { name: '锁定走廊尽头的苏晚' }).click()
  await page.getByRole('button', { name: '对焦苏晚手中的黑卡' }).click()
  await page.getByRole('button', { name: '指出玻璃右侧的延迟倒影' }).click()
  await page.getByRole('button', { name: '追踪仍亮着的红色门禁灯' }).click()
  ...
  await expect(page.locator('.speech-ui > button')).toBeInViewport()
  for (let index = 0; index < 3; index += 1) await page.locator('.speech-ui > button').click()
