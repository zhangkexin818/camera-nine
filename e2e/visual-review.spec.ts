...
async function finishPrologue(page: Page) {
  ...
  for (const record of ['工作证', '楼层扫描', '受损录像']) await page.getByRole('button', { name: record }).click()
  await page.getByRole('button', { name: /相信身体/ }).click()
  for (let index = 0; index < 3; index += 1) await page.locator('.speech-ui > button').click()
  await page.getByRole('button', { name: /保持拍摄/ }).click()
...
test('captures the exploration-first journey', async ({ page }) => {
  ...
  await shot(page, 'v8-identity-work-card.png')
  await page.getByRole('button', { name: '工作证' }).click()
  await page.getByRole('button', { name: '楼层扫描' }).click()
  await shot(page, 'v8-identity-floor-scan.png')
  await page.getByRole('button', { name: '受损录像' }).click()
  await shot(page, 'v8-identity-old-tape.png')
  await page.getByRole('button', { name: /相信身体/ }).click()
  await shot(page, 'v8-dialogue-zhou-calm.png')
  await page.locator('.speech-ui > button').click()
  await shot(page, 'v8-dialogue-su-warning.png')
  await page.locator('.speech-ui > button').click()
  await shot(page, 'v8-dialogue-zhou-confused.png')
  await page.locator('.speech-ui > button').click()
  await shot(page, 'v8-visual-action-choice.png')
  await page.getByRole('button', { name: /保持拍摄/ }).click()
  ...
  await page.getByRole('button', { name: /捡起落在门内的黑卡/ }).click()
  await shot(page, 'v8-dialogue-zhou-urgent.png')
  await page.locator('.speech-ui > button').click()
  await page.locator('.speech-ui > button').click()
  await shot(page, 'v8-dialogue-su-remember.png')
  await page.locator('.speech-ui > button').click()
  await expect(page.locator('.pixi-host')).toHaveAttribute('data-load-state', 'ready')
  ...
  await shot(page, 'v9-ending-answer-closeup.png')
  await page.getByRole('button', { name: '拉开镜头' }).click()
  await page.waitForTimeout(1700)
  await shot(page, 'v9-ending-answer-wide.png')
  await page.getByRole('button', { name: '确认空椅上的人' }).click()
  await shot(page, 'v9-ending-answer-revealed.png')

  await page.setViewportSize({ width: 390, height: 844 })
  await shot(page, 'v9-mobile-ending-answer.png')
  await page.reload()
